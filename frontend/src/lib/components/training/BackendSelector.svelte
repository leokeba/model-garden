<script lang="ts">
    ...
    let { backend, modelType, backends, loading, onBackendChange }: Props = $props();

    // Filter backends based on model type
    let availableBackends = $derived(
        backends.filter((b) => (modelType === 'vision' ? b.supports_vision : b.supports_text))
    );
...
<div>
    ...
    <div>
        <label for="backend" class="block text-sm font-medium text-gray-700 mb-2">
            Backend
        </label>
        {#if loading}
