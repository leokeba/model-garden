...
def example_2_specific_schema():
    ...
    completion = client.chat.completions.create(
        model="Qwen/Qwen2.5-3B-Instruct",
        messages=[{"role": "user", "content": "Tell me about the Golden Gate Bridge"}],
        response_format={
            "type": "json_schema",
            "json_schema": {"name": "LandmarkInfo", "schema": schema},
        },
    )

    # Validate response against schema
    landmark = LandmarkInfo.model_validate_json(completion.choices[0].message.content)
...
def example_4_function_calling():
    ...
    completion = client.chat.completions.create(
        ...
        response_format={
            "type": "json_schema",
            "json_schema": {"name": "FunctionCall", "schema": schema},
        },
    )

    function_call = FunctionCall.model_validate_json(completion.choices[0].message.content)
...
def example_5_batch_extraction():
    ...
    completion = client.chat.completions.create(
        ...
        response_format={
            "type": "json_schema",
            "json_schema": {"name": "ItemList", "schema": schema},
        },
    )

    items = ItemList.model_validate_json(completion.choices[0].message.content)
