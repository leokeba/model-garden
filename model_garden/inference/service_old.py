...
class InferenceService:
    ...
    async def load_model(self) -> None:
        ...
        try:
            ...
            # Check if this is a Mistral/Ministral model that needs native Mistral tokenizer
            is_mistral_model = any(
                x in self.model_path.lower() for x in ["mistral", "ministral", "pixtral"]
            )

            if is_mistral_model:
                # Use native Mistral tokenizer mode for proper image handling
                # See: https://huggingface.co/mistralai/Ministral-3-3B-Instruct-2512#vllm
                console.print(
                    "[cyan]🔧 Detected Mistral model - using native Mistral tokenizer mode[/cyan]"
                )
                engine_args_dict["tokenizer_mode"] = "mistral"
                engine_args_dict["config_format"] = "mistral"
                engine_args_dict["load_format"] = "mistral"
            elif is_vision:
            ...
            # Add LoRA support if enabled (but not for vision models - many don't support it in vLLM)
            if self.enable_lora and not is_vision:
                console.print(
            ...
            elif self.enable_lora and is_vision:
                console.print(
                    "[yellow]⚠️  LoRA support disabled for vision models (not supported by vLLM for most vision architectures)[/yellow]"
                )
    ...
    def _prepare_inputs(self, prompt: str, images: list[str] | None = None):
        ...
        try:
            ...
            # Pass PIL Image objects directly - this is the most compatible format
            # across different vision model architectures (Qwen2.5-VL, Pixtral, etc.)
            loaded_images = []
            for img_data in images:
                if img_data.startswith("data:image/"):
                    # It's a data URL with base64 - extract the base64 part
                    import re
                ...
                if img_data.startswith(("http://", "https://")):
                    ...
                    try:
                        ...
                        if img.mode != "RGB":
                            img = img.convert("RGB")
                        print(f"✅ Downloaded image from URL: {img.size} {img.mode}")
                    ...
                    try:
                        ...
                        if img.mode != "RGB":
                            img = img.convert("RGB")
                        print(f"✅ Loaded image from base64 data: {img.size} {img.mode}")
                        loaded_images.append(img)
                    except Exception as e:
                    ...
                    if img.mode != "RGB":
                        img = img.convert("RGB")
                    print(f"✅ Loaded image from file: {img.size} {img.mode}")
                    loaded_images.append(img)
    ...
    async def chat_completion(
        ...
        **kwargs,
        ...
        if image:
            ...
            # Do this regardless of tokenizer availability
    ...
    def _format_simple(self, messages: list[dict[str, Any]]) -> str:
        ...
        Args:
            messages: List of message dicts with 'role' and 'content' keys.
                     Content can be a string or a list of multimodal content parts.
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                # For Pixtral/Ministral, use [IMG] placeholder
                if has_image:
                    content = "[IMG]" + " ".join(text_parts)
                else:
