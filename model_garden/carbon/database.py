...
class EmissionsDatabase:
    ...
    def _ensure_db_exists(self) -> None:
        ...
        if not self.db_path.exists():
            self._write_db(
                {"version": "1.0", "created_at": datetime.now(UTC).isoformat(), "emissions": []}
            )
    ...
    def add_emission(self, emission_data: dict[str, Any]) -> None:
        ...
        if "timestamp" not in emission_data:
            emission_data["timestamp"] = datetime.now(UTC).isoformat()
        ...
        db["emissions"].sort(key=lambda x: x.get("timestamp", ""), reverse=True)

        self._write_db(db)

    def get_emission(self, job_id: str) -> dict[str, Any] | None:
        """
    ...
    def get_all_emissions(
        self, job_type: str | None = None, limit: int | None = None
    ) -> list[dict[str, Any]]:
        """
...
_emissions_db: EmissionsDatabase | None = None
