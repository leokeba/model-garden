...
def run_training_in_subprocess(job_config: dict) -> dict:
    ...
    from model_garden.training import ModelTrainer, VisionLanguageTrainer
    from model_garden.utils import cleanup_training_resources
    ...
    try:
        ...
        if is_vision:
            ...
            config = VisionTrainingConfig(
                ...
                selective_loss_masking_start_epoch=job_config.get(
            ...
            # Load datasets
            if from_hub:
                train_dataset = trainer.load_dataset_from_hub(
                    job_config["dataset_path"], split="train"
                )
            else:
            ...
            train_dataset = trainer.format_dataset(
                train_dataset,
                instruction_field=job_config["hyperparameters"].get(
...
def execute_training_job_in_subprocess(job_config: dict, timeout: int | None = None) -> dict:
    ...
    # Start subprocess
    process = ctx.Process(target=_subprocess_worker, args=(job_config, result_queue))
