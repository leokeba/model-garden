...
class SelectiveLossVisionCollator(UnslothVisionDataCollator):
    ...
    Example:
        ...
        >>> # Alternating masking (learn structure and semantics throughout)
        >>> collator = SelectiveLossVisionCollator(
        ...     model=model,
        ...     processor=processor,
        ...     mask_structural_tokens=True,
    ...
    def __call__(self, features):
        ...
        if self.current_step == 0 and self.verbose:
            console.print("[cyan]SelectiveLossVisionCollator processing batch[/cyan]")
        ...
        if self.current_step == 0 and self.verbose:
            console.print("[cyan]After parent.__call__:[/cyan]")
            if "labels" in batch:
        ...
        if "labels" in batch and len(batch["labels"]) > 0:
            ...
            if unmasked_tokens == 0:
                ...
                console.print(
                    f"[red]   Total tokens: {total_tokens}, Step: {self.current_step}[/red]"
                )
                ...
                if "input_ids" in batch and len(batch["input_ids"]) > 0:
                    try:
                        ...
                        if assistant_count == 0 and user_count > 0:
                            ...
                            console.print(
                                f"[red]   → SOLUTION: Increase max_seq_length (currently at {len(input_ids)})[/red]"
                            )
                            console.print(
                                "[red]   → For vision models with images, use 8192+ tokens[/red]"
                            )
                        else:
                            console.print(
                            ...
                            console.print(
                                "[red]   Check train_on_responses_only configuration[/red]"
                            )
        ...
        # Log when masking starts (only once for epoch_based, or on each transition for alternating)
        if self.masking_strategy == "epoch_based":
            if (
            ...
            # Track transitions for alternating mode
            if self.verbose and is_training:
                cycle_pos = self.current_step % self.mask_every_n_steps
                # Log at the start of each masking ON period
                if cycle_pos == 0 and self.current_step > 0:
                    console.print(
                        f"[green]🔄 Step {self.current_step}: Masking ON for next {self.mask_for_n_steps} steps[/green]"
                    )
                # Log at the start of each masking OFF period
                elif cycle_pos == self.mask_for_n_steps and self.current_step > 0:
                    off_steps = self.mask_every_n_steps - self.mask_for_n_steps
                    console.print(
                        f"[yellow]🔄 Step {self.current_step}: Masking OFF for next {off_steps} steps[/yellow]"
                    )
        ...
        if "labels" in batch:
            ...
            if is_training and self.batch_count == 0 and self.verbose:
                ...
                console.print(
                    f"   Prompt tokens (masked to -100): {prompt_masked} ({prompt_masked / total_tokens * 100:.1f}%)"
                )
                console.print(
                    f"   Assistant tokens (not -100): {total_tokens - prompt_masked} ({(total_tokens - prompt_masked) / total_tokens * 100:.1f}%)"
                )
    ...
    def _should_enable_masking(self) -> bool:
        ...
        - epoch_based: Enable masking after a certain epoch threshold
        - alternating: Alternate between masking ON and OFF every n steps

        Returns:
            True if masking should be enabled for current step/epoch
        """
        if self.masking_strategy == "epoch_based":
            current_epoch = self._get_current_epoch()
            ...
            # Masking is ON for the first mask_for_n_steps of each cycle
    ...
    def _get_current_epoch(self) -> float:
        ...
        if self.current_step > 0:
            ...
            if self.verbose and self.current_step % 50 == 0:
                console.print(
                    f"[yellow]Warning: Using estimated epoch {estimated_epoch:.2f} (trainer state unavailable)[/yellow]"
                )
    ...
    def set_trainer(self, trainer):
        ...
        if self.verbose and self.masking_start_epoch > 0.0:
            console.print(
                f"[cyan]✓ Trainer set for epoch-based masking (start: {self.masking_start_epoch})[/cyan]"
            )
    ...
    def _find_structural_indices(self, token_ids):
        ...
        try:
            full_text = self.processor.tokenizer.decode(valid_tokens, skip_special_tokens=False)
        except Exception as e:
    ...
    def _print_unmasked_sample(self, masked_labels, original_labels):
        ...
        try:
            ...
            if self.batch_count == 10:  # Only on first verbose batch to avoid spam
                console.print(
                    "[yellow]  🔍 DEBUG: Checking original labels (what Unsloth gave us):[/yellow]"
                )
                ...
                console.print(
                    f"[yellow]     Masked by Unsloth (prompt): {original_masked} ({original_masked / total_tokens * 100:.1f}%)[/yellow]"
                )
                console.print(
                    f"[yellow]     Unmasked (assistant): {original_unmasked} ({original_unmasked / total_tokens * 100:.1f}%)[/yellow]"
                )
                ...
                if unmasked_before.any():
                    ...
                    console.print(
                        f"[yellow]     First 100 unmasked tokens (should be assistant only): {repr(original_decoded[:300])}[/yellow]"
                    )
                else:
                    console.print("[yellow]     WARNING: No unmasked tokens found![/yellow]")
            ...
            unmasked_text = self.processor.tokenizer.decode(
                unmasked_tokens, skip_special_tokens=True
            )
            ...
            console.print(
                f"[dim]  └─ Unmasked content ({kept_pct:.1f}% kept, {kept_tokens}/{valid_original_count} tokens):[/dim]"
            )
            ...
            if len(original_text) <= 400:
                console.print(
                    f"[dim]     Original ({len(original_text)} chars): {repr(original_text[:200])}{'...' if len(original_text) > 200 else ''}[/dim]"
                )
...
def detect_schema_keys_from_dataset(
    dataset, processor, num_samples: int = 50, threshold: float = 0.3, verbose: bool = False
) -> set[str]:
...
def create_selective_loss_collator(
    ...
    Example:
        >>> # Epoch-based masking (recommended for initial exploration)
        >>> collator = create_selective_loss_collator(
        ...     model, processor,
        ...     mask_level="aggressive",
        ...     masking_strategy="epoch_based",
        ...
        >>> # Alternating masking (recommended for balanced learning)
        >>> collator = create_selective_loss_collator(
        ...     model, processor,
        ...     mask_level="aggressive",
        ...     masking_strategy="alternating",
