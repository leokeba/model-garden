...
class JobQueue:
    ...
    def _save_queue(self) -> None:
        ...
        try:
            ...
            with open(self._storage_file, "w") as f:
                json.dump(
                    {"queue": self._queue, "updated_at": datetime.now(UTC).isoformat()}, f, indent=2
                )
        except Exception as e:
            print(f"Warning: Could not save job queue: {e}")

    async def add_job(
        self, job_id: str, job_type: str, job_config: dict[str, Any], priority: int = 0
    ) -> None:
        """
    ...
    async def complete_job(self, job_id: str, result: dict[str, Any] | None = None) -> bool:
        """
    ...
    async def list_jobs(
        self, status: JobStatus | None = None, job_type: str | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    async def update_job_status_message(self, job_id: str, message: str) -> bool:
        """
    ...
    async def has_running_job(self, job_type: JobType | None = None) -> bool:
        """
        ...
        running_jobs = await self.list_jobs(
            status=JobStatus.RUNNING, job_type=job_type.value if job_type else None
        )
        return len(running_jobs) > 0

    async def get_running_job(self, job_type: JobType | None = None) -> dict[str, Any] | None:
        """
    ...
    async def get_next_job_by_type(self, job_type: JobType) -> dict[str, Any] | None:
        """
        ...
        """
        queued_jobs = await self.list_jobs(status=JobStatus.QUEUED, job_type=job_type.value)
